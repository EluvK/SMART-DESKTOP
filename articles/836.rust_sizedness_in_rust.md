...
实际的应用场景：标准库里的容易，`HashSet` 是通过 `HashMap` 实现的： [hashbrown/set.rs](https://github.com/rust-lang/hashbrown/blob/v0.12.0/src/set.rs#L115)
``` RUST
...
注意需要写上 `#![feature(never_type)]` ，因为目前还是实验性标准。
...
译注：原文在这里用一个去除结构体 `Send` 和 `Sync` 的Trait的例子来说明其作用。
